  ...
  const onFormSubmit = async (values, setSubmitting) => {
    const body = {
      ...
      data: {
        ...
        userpw: values.newPassword,
      },
      usercode: data.usercode,
      event: "userpasswordpdate",
      action: "update",
    };
    const databody = encryptMessage(body);
    const response = await UpdateUserPassword({ body: databody });
    if (response.status == "success") {
      ...
      toast.push(
        <Notification title={"Error in Password updatation"} type="error" />,
        {
          placement: "top-center",
        }
      );
